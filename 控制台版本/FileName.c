...
int wide=10, high=20, speed = SPEED1, page_theme = 1, sound = 1, tetris_theme = 1, line = 1;
...
void welcome_page(void)
{
	...
	for (;;)
	{
		for (int key; key = key_check();)
		{
			...
			if (key == 8)
			{
				wide = 10,high = 20,speed = SPEED1,page_theme = 1,sound = 1,line = 1,tetris_theme = 1,score = 0,history_score = 0,round = 0;
				for (int i = 0; i < 999; i++)
...
void failure_page(void)
{
	...
	printf("\n-------------------------------------\n");
	print_version();
	if ((score_record[round] = score) > history_score)history_score = score;
	fprintf(fopen("game.txt", "a"), "round:%d\t\tmode:%d\t\tscore:%d\n", round, mode_record[round], score_record[round]);
	for (;;)
	{
		int key = key_check();
		if (key == 15)welcome_page();
		if (key == 10)score_board();
	}
}
...
void game(void)
{
	mode_record[++round] = (speed == SPEED1) ? 1 : (speed == SPEED2 ? 2 : 3);//部分参数重置
	score = 0, kill_line = 0, end_game_time = 0;
	for (int y = 0; y < high + 1; y++)//pixel重置
		for (int x = 0; x < wide + 2; x++)
			pixel[x][y] = 0;
	for (int x = 0; x < wide + 2; x++)//三围筑墙
		pixel[x][high] = -1;
	for (int y = 0; y < high + 1; y++)
		pixel[0][y] = pixel[wide + 1][y] = -1;
	shape = my_random(13);//生成第一个shape
	print_game();
	for (;;)
	{
		next_shape = my_random(13);
		print_next_shape();
		new_shape(my_random(4),my_random(wide-3));
		for (;;)
		{
			int r=0,k=0,key = key_check();
			if (key == 15)while (move(3));
			else if (key == 11)k = r = move(1);
			else if (key == 12)k = r = move(2);
			else if (key == 13)r = change_shape();
			else if (key == 14)k = r = move(3);
...
void print_game(void)
{
	...
	gotoxy(wide * 2, 15);printf("\tS \t快速下移");
	gotoxy(wide * 2, 16);printf("\tW \t改变形状");
	gotoxy(wide * 2, 17);printf("\tSPACE\t下移到底");
...
void print_next_shape(void)
{
	gotoxy(14, 1);printf("%s", speed == SPEED1 ? "标 准" : (speed == SPEED2 ? "挑 战" : "极 限"));
...
void print_pixel(void)
{
	...
	for (int y = 0; y < high; y++)
		for (int x = 1; x < wide + 1; x++)
		{
			...
			if (pixel[x][y] == 1 || pixel[x][y] == -1)printf("\033[31m%s\033[37m\033[1m", tetris_theme == 1 ? "囗" : (tetris_theme == 2 ? "浙" : "王"));
			if (pixel[x][y] == 2 || pixel[x][y] == -2)printf("\033[33m%s\033[37m\033[1m", tetris_theme == 1 ? "囗" : (tetris_theme == 2 ? "江" : "王"));
			if (pixel[x][y] == 3 || pixel[x][y] == -3)printf("\033[34m%s\033[37m\033[1m", tetris_theme == 1 ? "囗" : (tetris_theme == 2 ? "大" : "袁"));
			if (pixel[x][y] == 4 || pixel[x][y] == -4)printf("\033[35m%s\033[37m\033[1m", tetris_theme == 1 ? "囗" : (tetris_theme == 2 ? "学" : "袁"));
			if (x == wide )printf("\n");
		}
}
...
//1~9返回1~9，0返回10，左键返回11，右键返回12，上键返回13，下键返回14，空格返回15，无输入返回0
...
int main()
{
	SetConsoleTitle(L"俄罗斯方块游戏");//窗囗标题
	HWND hwnd = GetForegroundWindow();//窗囗全屏
	...
	for (; !feof(fp);)
	{
		fscanf_s(fp, "round:%d\t\t", &round);
		fscanf_s(fp, "mode:%d\t\tscore:%d\n", &mode_record[round], &score_record[round]);
		score = score_record[round];
